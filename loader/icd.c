...
void khrIcdLayerAdd(const char *libraryName)
{
    ...
    if (!p_clInitLayer && !p_clInitLayerWithProperties)
    {
        KHR_ICD_TRACE("failed to get an initialization function\n");
        goto Done;
    }
    ...
    if (p_clInitLayerWithProperties && p_clDeinitLayer)
    {
        const cl_layer_properties properties[] =
            {CL_LAYER_DEINITIALIZATION, CL_LAYER_DEINITIALIZATION_FUNCTION,
        ...
        result = p_clInitLayerWithProperties(
            ...
            &layerDispatch,
            properties);
    } else if (p_clInitLayerWithProperties)
    {
        const cl_layer_properties properties[] =
            {CL_LAYER_DEINITIALIZATION, CL_LAYER_DEINITIALIZATION_ATEXIT,
             CL_LAYER_PROPERTIES_LIST_END};
        result = p_clInitLayerWithProperties(
            loaderDispatchNumEntries,
            targetDispatch,
